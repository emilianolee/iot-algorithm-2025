...
if __name__ == '__main__':
    ...
    for name in nameAry[1:]:
        ...
        current = root
        while True:
            if name < current.data: # 현재 name인 노드 안 데이터보다 작으면, 왼쪽으로
    ...
    while True:
        if delName == current.data: # 삭제할 때
            ...
            elif current.left != None and current.right == None: # 왼쪽 노드에 자식이 있고, 오른쪽에 없을 때
                if parent.left == current:
            ...
            elif current.left != None and current.right == None: # 오른쪽 노드에 자식이 있고, 왼쪽에 없을 때
                if parent.left == current:
